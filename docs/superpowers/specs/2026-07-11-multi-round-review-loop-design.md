...
queued ──► running ──► reviewing ──┬── PASS + N≥3 ──► reconcile ──► succeeded
   (implementer       (reviewer     │
    or fixer)          agent)       ├── PASS + N<3 ──► reviewing (再审查)
                                      │
...
`running` 状态被 implementer 和 fixer 复用（设计文档明确允许）。区分依据：
...
def _run_to_dict(run: Run) -> dict:
    ...
    # review_history 内部的 datetime 也要转
    for rec in d.get('review_history', []):
        for key in ['timestamp', 'reviewer_started_at', 'reviewer_finished_at']:
            if rec.get(key):
                rec[key] = rec[key].isoformat() if isinstance(rec[key], datetime) else rec[key]
    return d
```
...
- `_format_feedback_text(items, summary) -> str` — JSON feedback 数组 → 多行文本
- `_malformed_result(iteration, msg) -> ReviewResult` — 构造 FAIL fallback

### 5.3 Fixer 复用 `symphony-worker`

不新建 fixer agent。fixer 是 `symphony-worker` + 不同 prompt（带审查反馈）。同一个 agent 文件，不同的 `.san/skills/fix-{N}.md` prompt。
...
def process_completed(runs: list[Run], cfg) -> None:
    ...
    for run in runs:
        ...
        if run.status == "running":
            _on_implementer_done(run, cfg)
        elif run.status == "reviewing":
            _on_reviewer_done(run, cfg)
```

### 7.2 `_on_implementer_done(run, cfg)`

implementer/fixer 退出 → 启动 reviewer。

```python
def _on_implementer_done(run: Run, cfg) -> None:
    try:
...
Soft warning（不阻止启动）：扫 `~/.config/opencode/opencode.jsonc`，若 `cfg.agent.reviewer.extra_args` 不含 `--model`，列出可用 provider/model 并建议补到 WORKFLOW.md。
...
| 测试文件 | 覆盖 |
|---------|------|
| `tests/test_state.py` (扩展) | `Run` 含新字段 round-trip JSONC；旧 JSONC 不含新字段时用默认值 |
| `tests/test_reviewer.py` (新) | `parse_review_result` 5 路径：valid PASS / valid FAIL / 文件缺失 / JSON parse 失败 / verdict 非法；`_format_feedback_text` 格式 |
| `tests/test_orchestrator.py` (扩展) | `_on_reviewer_done` 决策表 4 分支；`_on_implementer_done` dispatch_review 成功/失败；`process_completed` 路由 |
...
| `tests/test_config.py` (扩展) | `_dict_to_config` 解析 `agent.reviewer` 子节点；缺 `reviewer` 节点时用默认值 |
| `tests/test_bootstrap.py` (扩展) | `check_reviewer_model` soft warning 各分支 |
